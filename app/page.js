...
function TrackerApp(props){
  ...
  function Sidebar(){var pCos=cos.filter(function(c){return(c.status||"portfolio")===sideTab});
    ...
    {sideHover==="portfolio"&&!isMobile&&<div style={{position:"fixed",left:(isMobile?280:240),top:flyY,background:K.card,border:"1px solid "+K.bdr,borderRadius:8,padding:"6px 0",boxShadow:"0 4px 16px rgba(0,0,0,.2)",zIndex:9999,minWidth:160}} onMouseEnter={function(){setSideHover("portfolio")}} onMouseLeave={function(){setSideHover(null)}}>
      {[{l:"Portfolio",pg:"dashboard",icon:"overview",min:0},{l:"Analytics",pg:"analytics",icon:"bar",min:2},{l:"Earnings Calendar",pg:"calendar",icon:"target",min:2},{l:"Dividends",pg:"dividends",icon:"dollar",min:2},{l:"Timeline",pg:"timeline",icon:"trending",min:2}].filter(function(sub){return maturity>=sub.min}).map(function(sub){return<div key={sub.pg} onClick={navClick(function(){setSelId(null);setPage(sub.pg);setSideHover(null)})} style={{padding:"8px 16px",cursor:"pointer",fontSize:11,color:K.mid,fontFamily:fm,display:"flex",alignItems:"center",gap:8}} onMouseEnter={function(e){e.currentTarget.style.background=K.acc+"10"}} onMouseLeave={function(e){e.currentTarget.style.background="transparent"}}><IC name={sub.icon} size={12} color={K.dim}/>{sub.l}</div>})}</div>}</div>
    {maturity>=1&&<div style={{position:"relative"}} onMouseEnter={function(e){setSideHover("hub");setFlyY(e.currentTarget.getBoundingClientRect().top)}} onMouseLeave={function(){setSideHover(null)}}>
    ...
    {maturity>=1&&<div style={{padding:"12px 20px",cursor:"pointer",background:page==="review"?K.grn+"10":"transparent",borderLeft:page==="review"?"2px solid "+K.grn:"2px solid transparent"}} onClick={navClick(function(){setSelId(null);setPage("review")})}><span style={{fontSize:12,color:page==="review"?K.grn:sideMid,fontWeight:page==="review"?600:400,fontFamily:fm,display:"flex",alignItems:"center",gap:8}}><IC name="shield" size={14} color={page==="review"?K.grn:sideMid}/>Weekly Review{!currentWeekReviewed&&<span style={{width:6,height:6,borderRadius:"50%",background:K.grn,display:"inline-block"}}/>}</span></div>}
    {maturity>=2&&<div style={{padding:"12px 20px",cursor:"pointer",background:page==="assets"?K.amb+"10":"transparent",borderLeft:page==="assets"?"2px solid "+K.amb:"2px solid transparent"}} onClick={navClick(function(){setSelId(null);setPage("assets")})}><span style={{fontSize:12,color:page==="assets"?K.amb:sideMid,fontWeight:page==="assets"?600:400,fontFamily:fm,display:"flex",alignItems:"center",gap:8}}><IC name="dollar" size={14} color={page==="assets"?K.amb:sideMid}/>All Assets</span></div>}
    ...
    <div style={{flex:1,overflowY:"auto",paddingTop:4}}>{pCos.map(function(c){var active=selId===c.id,h=gH(c.kpis),d=dU(c.earningsDate);
      ...
      return<div key={c.id} className="ta-side-item" style={{padding:"10px 16px 10px 18px",cursor:"pointer",display:"flex",alignItems:"center",gap:10,background:active?K.blue+"10":"transparent",borderLeft:active?"2px solid "+K.blue:"2px solid transparent"}} onClick={navClick(function(){setSelId(c.id);setExpKpi(null);setSubPage(null);setDetailTab("dossier");setPage("dashboard")})}>
        ...
        <div style={{display:"flex",flexDirection:"column",alignItems:"flex-end",gap:2}}>
          ...
          <div style={{width:6,height:6,borderRadius:"50%",background:h.c}}/></div></div>})}</div>
  ...
  function Dashboard(){var filtered=cos.filter(function(c){return(c.status||"portfolio")===sideTab});
    ...
    {/* ── Progressive Journey — teaser for locked features ── */}
    {sideTab==="portfolio"&&maturity<2&&portCount>0&&<div style={{background:K.card,border:"1px solid "+K.bdr,borderRadius:12,padding:"24px 28px",marginBottom:28}}>
      <div style={{fontSize:14,fontWeight:600,color:K.txt,fontFamily:fh,marginBottom:6}}>Your ThesisAlpha grows with you</div>
      <div style={{fontSize:12,color:K.mid,lineHeight:1.7,marginBottom:16}}>As you build your ownership practice, new tools will unlock. Focus on the fundamentals first — the rest follows.</div>
      <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10}}>
        {[{icon:"lightbulb",label:"Write your thesis",desc:"Define why you own each company",done:portCount>=1&&cos.some(function(c){return c.thesisNote&&c.thesisNote.trim().length>20}),req:"Step 1"},
          {icon:"target",label:"Track KPIs",desc:"Set the metrics that prove your thesis",done:cos.some(function(c){return c.kpis.length>=2}),req:"Step 2"},
          {icon:"overview",label:"Add 2+ holdings",desc:"Build a real portfolio to track",done:portCount>=2,req:"Unlocks: Analytics, Calendar, Dividends"},
          {icon:"shield",label:"Weekly Review",desc:"Confirm conviction across your portfolio",done:hasReviewed,req:"Unlocks: Timeline, All Assets, Owner's Hub"}
        ].map(function(step){return<div key={step.label} style={{display:"flex",alignItems:"flex-start",gap:10,padding:"12px 14px",borderRadius:8,background:step.done?K.grn+"06":K.bg,border:"1px solid "+(step.done?K.grn+"20":K.bdr)}}>
          {step.done?<svg width="18" height="18" viewBox="0 0 16 16" style={{flexShrink:0,marginTop:1}}><circle cx="8" cy="8" r="7" fill={K.grn+"20"} stroke={K.grn} strokeWidth="1.5"/><path d="M5 8l2 2 4-4" stroke={K.grn} strokeWidth="1.5" fill="none"/></svg>
            :<div style={{width:18,height:18,borderRadius:"50%",border:"1.5px solid "+K.bdr,display:"flex",alignItems:"center",justifyContent:"center",flexShrink:0,marginTop:1}}><IC name={step.icon} size={10} color={K.dim}/></div>}
          <div><div style={{fontSize:11,fontWeight:600,color:step.done?K.grn:K.txt}}>{step.label}</div>
            <div style={{fontSize:10,color:K.dim,marginTop:2}}>{step.done?step.desc:step.req}</div></div></div>})}</div></div>}
