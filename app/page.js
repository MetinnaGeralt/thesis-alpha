...
var MOAT_TYPES=[{id:"brand",label:"Brand Power",icon:"\u2605",desc:"Strong brand recognition, pricing power, customer loyalty"},{id:"switching",label:"Switching Costs",icon:"\u26D3",desc:"High cost or friction to move to a competitor"},{id:"network",label:"Network Effects",icon:"\u26A1",desc:"Product value increases as more users join"},{id:"cost",label:"Cost Advantage",icon:"\u2193",desc:"Structural cost advantage vs competitors"},{id:"ip",label:"IP / Patents",icon:"\u2694",desc:"Protected technology, patents, trade secrets"},{id:"regulatory",label:"Regulatory Moat",icon:"\u2696",desc:"Licenses, regulations, barriers to entry"},{id:"scale",label:"Scale Economy",icon:"\u25B2",desc:"Economies of scale that deter new entrants"}];
var MSTAR_RATINGS=["Wide","Narrow","None","Not Rated"];
var MOAT_TRENDS=["Strengthening","Stable","Eroding"];
var DECISION_TYPES=[{id:"buy",label:"BUY",color:"#22C55E"},{id:"sell",label:"SELL",color:"#EF4444"},{id:"add",label:"ADD",color:"#22C55E"},{id:"trim",label:"TRIM",color:"#F59E0B"},{id:"hold",label:"HOLD",color:"#6366F1"}];
var OUTCOME_TAGS=[{id:"good",label:"Good Call",color:"#22C55E",icon:"\u2713"},{id:"bad",label:"Bad Call",color:"#EF4444",icon:"\u2717"},{id:"lucky",label:"Lucky",color:"#F59E0B",icon:"\u2606"},{id:"unlucky",label:"Unlucky",color:"#9333EA",icon:"\u2193"},{id:"tbd",label:"Too Early",color:"#6B7280",icon:"\u23F3"}];
function calcOwnerScore(cos){var totalP=0,maxP=0;
  ...
  var thesisP=0,thesisMax=20;portfolio.forEach(function(c){var score=0;
    if(c.thesisNote&&c.thesisNote.length>20)score+=3;
    if(c.thesisVersions&&c.thesisVersions.length>1)score+=1;
    if(c.thesisSections){if(c.thesisSections.believe)score+=1;if(c.thesisSections.whyNow)score+=1;if(c.thesisSections.risks)score+=1;if(c.thesisSections.change)score+=1}
    thesisP+=Math.min(score,5)});
  thesisP=Math.round(thesisP/portfolio.length/5*thesisMax);
  // 2. KPI discipline (20 pts)
  var kpiP=0,kpiMax=20;portfolio.forEach(function(c){var score=0;
    if(c.kpis&&c.kpis.length>=2)score+=2;
    if(c.kpis&&c.kpis.length>=4)score+=1;
    var tracked=c.kpis?c.kpis.filter(function(k){return k.lastResult}).length:0;
    if(tracked>0)score+=2;
    if(c.earningsHistory&&c.earningsHistory.length>=2)score+=1;
    kpiP+=Math.min(score,5)});
  kpiP=Math.round(kpiP/portfolio.length/5*kpiMax);
  // 3. Journal consistency (20 pts)
  var jrnP=0,jrnMax=20;portfolio.forEach(function(c){var score=0;
    var decisions=c.decisions||[];
    if(decisions.length>=1)score+=2;
    if(decisions.length>=3)score+=1;
    var withReason=decisions.filter(function(d){return d.reasoning&&d.reasoning.length>10}).length;
    if(withReason>=1)score+=1;
    var withOutcome=decisions.filter(function(d){return d.outcome&&d.outcome!=="tbd"}).length;
    if(withOutcome>=1)score+=1;
    jrnP+=Math.min(score,5)});
  jrnP=Math.round(jrnP/portfolio.length/5*jrnMax);
  // 4. Conviction hygiene (15 pts)
  var convP=0,convMax=15;portfolio.forEach(function(c){var score=0;
    if(c.conviction>0)score+=2;
    if(c.convictionHistory&&c.convictionHistory.length>=2)score+=2;
    if(c.biasChecklist&&c.biasChecklist.completedAt)score+=1;
    convP+=Math.min(score,5)});
  convP=Math.round(convP/portfolio.length/5*convMax);
  // 5. Moat vigilance (10 pts)
  var moatP=0,moatMax=10;portfolio.forEach(function(c){var score=0;
    if(c.moat&&c.moat.types&&c.moat.types.length>0)score+=2;
    if(c.moat&&c.moat.strength)score+=1;
    if(c.moat&&c.moat.trend)score+=1;
    if(c.moat&&c.moat.morningstar)score+=1;
    moatP+=Math.min(score,5)});
  moatP=Math.round(moatP/portfolio.length/5*moatMax);
  // 6. Portfolio balance (15 pts)
  var balP=0,balMax=15;
  if(portfolio.length>=3)balP+=5;else if(portfolio.length>=2)balP+=3;
  var sectors={};portfolio.forEach(function(c){var s=c.sector||"Other";sectors[s]=(sectors[s]||0)+1});
  var maxConc=Math.max.apply(null,Object.values(sectors))/portfolio.length;
  if(maxConc<=0.4)balP+=5;else if(maxConc<=0.6)balP+=3;
  var styles={};portfolio.forEach(function(c){if(c.investStyle)styles[c.investStyle]=(styles[c.investStyle]||0)+1});
  if(Object.keys(styles).length>=2)balP+=5;else if(Object.keys(styles).length>=1)balP+=3;
  balP=Math.min(balP,balMax);
  totalP=thesisP+kpiP+jrnP+convP+moatP+balP;
  return{total:totalP,breakdown:{thesis:thesisP,kpi:kpiP,journal:jrnP,conviction:convP,moat:moatP,balance:balP},max:100}}
var SAMPLE=[{id:1,ticker:"NVDA",name:"NVIDIA Corporation",sector:"Semiconductors",domain:"nvidia.com",irUrl:"https://investor.nvidia.com",earningsDate:"2026-02-26",earningsTime:"AMC",lastChecked:null,notes:"",sourceUrl:"https://investor.nvidia.com",sourceLabel:"Q4 FY26",earningSummary:"Data Center revenue surged 93% YoY to $39.2B.",thesisNote:"AI capex cycle still early innings.",position:{shares:50,avgCost:128.5},conviction:9,convictionHistory:[{date:"2025-06-01",rating:8,note:"Strong but expensive"},{date:"2025-11-20",rating:9,note:"Data center demand insatiable"},{date:"2026-01-15",rating:9,note:"AI capex still accelerating"}],status:"portfolio",docs:[{id:1,title:"Core Thesis: AI Infrastructure",folder:"why-i-own",content:"NVIDIA is the picks-and-shovels play on AI. Data center GPU demand is insatiable.",updatedAt:"2026-01-15T10:00:00Z"}],earningsHistory:[{quarter:"Q3 2025",summary:"Revenue $35.1B (+94% YoY). Data Center $30.8B. Gross margin 74.6%.",results:[{kpi_name:"Data Center Revenue",actual_value:30.8,status:"met",excerpt:"Data Center $30.8B"},{kpi_name:"Gross Margin",actual_value:74.6,status:"met",excerpt:"GAAP GM 74.6%"}],sourceUrl:"https://investor.nvidia.com",sourceLabel:"NVIDIA Press Release",checkedAt:"2025-11-20T18:00:00Z"},{quarter:"Q2 2025",summary:"Revenue $30.0B (+122% YoY). Data Center $26.3B. Gross margin 75.1%.",results:[{kpi_name:"Data Center Revenue",actual_value:26.3,status:"met",excerpt:"Data Center $26.3B"},{kpi_name:"Gross Margin",actual_value:75.1,status:"met",excerpt:"GAAP GM 75.1%"}],sourceUrl:"https://investor.nvidia.com",sourceLabel:"NVIDIA Press Release",checkedAt:"2025-08-28T18:00:00Z"}],moat:{types:["ip","scale","switching"],strength:"wide",trend:"Strengthening",morningstar:"Wide",mstarSources:["IP / Patents","Switching Costs"],notes:"CUDA ecosystem + data center GPU dominance"},
    thesisVersions:[{date:"2025-06-01",text:"AI capex cycle early innings. NVIDIA dominates GPU compute."},{date:"2026-01-15",text:"AI capex cycle still early innings. Data center demand insatiable."}],
    thesisSections:{believe:"NVIDIA is the picks-and-shovels play on AI infrastructure.",whyNow:"AI capex cycle accelerating, hyperscaler spend increasing QoQ.",risks:"Customer concentration, custom silicon (TPUs, Trainium), China export controls.",change:"If major hyperscalers shift >30% of GPU spend to custom silicon."},
    decisions:[{id:1,type:"buy",date:"2024-11-20",price:140,shares:30,reasoning:"Post-earnings dip, data center thesis intact",outcome:"good",outcomeNote:"Position up 60%+ since entry"},{id:2,type:"add",date:"2025-06-15",price:128.5,shares:20,reasoning:"Added on pullback, conviction increased after GTC keynote",outcome:"good",outcomeNote:"Thesis playing out, AI capex accelerating"}],
    biasChecklist:{completedAt:"2024-11-18",flags:["confirmation_bias"],notes:"Checked: Am I anchoring to AI hype? Reviewed bear case."},
    kpis:[{id:1,name:"Data Center Revenue",target:"\u226535B",rule:"gte",value:35,unit:"B",period:"Q4 FY26",notes:"",lastResult:{actual:39.2,status:"met",excerpt:"Data Center revenue was $39.2B."}},{id:2,name:"Gross Margin",target:"\u226573%",rule:"gte",value:73,unit:"%",period:"Q4 FY26",notes:"GAAP",lastResult:{actual:73.5,status:"met",excerpt:"GAAP gross margin was 73.5%."}}]},{id:2,ticker:"CRWD",name:"CrowdStrike",sector:"Cybersecurity",domain:"crowdstrike.com",irUrl:"https://ir.crowdstrike.com",earningsDate:"2026-03-04",earningsTime:"AMC",lastChecked:null,notes:"",sourceUrl:null,sourceLabel:null,earningSummary:null,thesisNote:"Post-outage recovery.",position:{shares:0,avgCost:0},conviction:6,convictionHistory:[{date:"2025-09-01",rating:5,note:"Outage fallout"},{date:"2026-01-10",rating:6,note:"Recovery underway"}],status:"watchlist",docs:[],earningsHistory:[],moat:{types:["switching","network"],strength:"narrow",trend:"Stable",morningstar:"Narrow",mstarSources:["Switching Costs"],notes:"Agent-based architecture creates deep integration"},
    thesisVersions:[{date:"2026-01-10",text:"Post-outage recovery. Watching churn and new ARR closely."}],
    thesisSections:{believe:"CrowdStrike has best-in-class endpoint security with high switching costs.",whyNow:"Post-outage selloff creates potential entry if recovery data confirms.",risks:"Outage fallout on enterprise trust, rising competition from SentinelOne.",change:"If gross retention falls below 95% for two consecutive quarters."},
    decisions:[],
    biasChecklist:null,
    kpis:[{id:1,name:"Net New ARR",target:"\u2265220M",rule:"gte",value:220,unit:"M",period:"Q4 FY26",notes:"",lastResult:null},{id:2,name:"Gross Retention",target:"\u226595%",rule:"gte",value:95,unit:"%",period:"Q4 FY26",notes:"",lastResult:null}]}];
...
function TrackerApp(props){
  ...
  useEffect(function(){ldS("ta-data").then(function(d){if(d&&d.cos){setCos(d.cos.map(function(c){return Object.assign({docs:[],earningsHistory:[],position:{shares:0,avgCost:0,currentPrice:0},conviction:0,convictionHistory:[],status:"portfolio",industry:"",lastDiv:0,divPerShare:0,divFrequency:"quarterly",exDivDate:"",moat:{types:[],strength:"",trend:"",morningstar:"",mstarSources:[],notes:""},thesisVersions:[],thesisSections:{believe:"",whyNow:"",risks:"",change:""},decisions:[],biasChecklist:null},c)}));}if(d&&d.notifs)setNotifs(d.notifs);setLoaded(true)})},[]);
  ...
  function MoatModal(){if(!sel)return null;var m=sel.moat||{types:[],strength:"",trend:"",morningstar:"",mstarSources:[],notes:""};
    var _f=useState({types:(m.types||[]).slice(),strength:m.strength||"",trend:m.trend||"",morningstar:m.morningstar||"",mstarSources:(m.mstarSources||[]).slice(),notes:m.notes||""}),f=_f[0],setF=_f[1];
    function toggleType(id){setF(function(p){var t=p.types.slice();var i=t.indexOf(id);if(i>=0)t.splice(i,1);else t.push(id);return Object.assign({},p,{types:t})})}
    function toggleMstar(id){setF(function(p){var s=p.mstarSources.slice();var i=s.indexOf(id);if(i>=0)s.splice(i,1);else s.push(id);return Object.assign({},p,{mstarSources:s})})}
    return<Modal title={"Moat Assessment \u2014 "+sel.ticker} onClose={function(){setModal(null)}} w={580} K={K}>
      <div style={{marginBottom:20}}>
        <label style={{display:"block",fontSize:11,color:K.dim,marginBottom:8,letterSpacing:.5,textTransform:"uppercase",fontFamily:fm}}>Your Moat Sources</label>
        <div style={{display:"flex",flexWrap:"wrap",gap:6}}>{MOAT_TYPES.map(function(mt){var isSel=f.types.indexOf(mt.id)>=0;return<button key={mt.id} onClick={function(){toggleType(mt.id)}} style={{display:"inline-flex",alignItems:"center",gap:4,padding:"6px 12px",borderRadius:6,border:"1px solid "+(isSel?K.grn+"60":K.bdr),background:isSel?K.grn+"12":"transparent",color:isSel?K.grn:K.mid,fontSize:11,cursor:"pointer",fontFamily:fm,fontWeight:isSel?600:400}}>{mt.icon} {mt.label}</button>})}</div></div>
      <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:"0 12px"}}>
        <Sel label="Moat Width" value={f.strength} onChange={function(v){setF(function(p){return Object.assign({},p,{strength:v})})}} options={[{v:"",l:"Not Assessed"},{v:"wide",l:"Wide Moat"},{v:"narrow",l:"Narrow Moat"},{v:"none",l:"No Moat"}]} K={K}/>
        <Sel label="Moat Trend" value={f.trend} onChange={function(v){setF(function(p){return Object.assign({},p,{trend:v})})}} options={[{v:"",l:"Not Set"},{v:"Strengthening",l:"\u25B2 Strengthening"},{v:"Stable",l:"\u2500 Stable"},{v:"Eroding",l:"\u25BC Eroding"}]} K={K}/>
      </div>
      <div style={{borderTop:"1px solid "+K.bdr,paddingTop:16,marginTop:4,marginBottom:8}}><div style={{fontSize:11,color:K.dim,letterSpacing:2,textTransform:"uppercase",fontFamily:fm,marginBottom:12}}>Morningstar Reference</div></div>
      <Sel label="Morningstar Rating" value={f.morningstar} onChange={function(v){setF(function(p){return Object.assign({},p,{morningstar:v})})}} options={[{v:"",l:"Not Set"},{v:"Wide",l:"\u2605 Wide Moat"},{v:"Narrow",l:"\u2605 Narrow Moat"},{v:"None",l:"No Moat"},{v:"Not Rated",l:"Not Rated"}]} K={K}/>
      {f.morningstar&&f.morningstar!=="Not Rated"&&f.morningstar!=="None"&&<div style={{marginBottom:16}}>
        <label style={{display:"block",fontSize:11,color:K.dim,marginBottom:8,letterSpacing:.5,textTransform:"uppercase",fontFamily:fm}}>Morningstar Moat Sources</label>
        <div style={{display:"flex",flexWrap:"wrap",gap:6}}>{MOAT_TYPES.map(function(mt){var isSel=f.mstarSources.indexOf(mt.label)>=0;return<button key={mt.id} onClick={function(){toggleMstar(mt.label)}} style={{display:"inline-flex",alignItems:"center",gap:4,padding:"5px 10px",borderRadius:6,border:"1px solid "+(isSel?K.blue+"60":K.bdr),background:isSel?K.blue+"12":"transparent",color:isSel?K.blue:K.dim,fontSize:10,cursor:"pointer",fontFamily:fm}}>{mt.icon} {mt.label}</button>})}</div></div>}
      <Inp label="Notes" value={f.notes} onChange={function(v){setF(function(p){return Object.assign({},p,{notes:v})})}} ta placeholder="Moat analysis notes..." K={K}/>
      {f.types.length>0&&f.morningstar&&<div style={{background:K.bg,border:"1px solid "+K.bdr,borderRadius:8,padding:14,marginBottom:16}}>
        <div style={{fontSize:10,color:K.dim,letterSpacing:1,textTransform:"uppercase",fontFamily:fm,marginBottom:8}}>Your Assessment vs Morningstar</div>
        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:12}}>
          <div><div style={{fontSize:11,color:K.mid,marginBottom:4}}>You say</div><div style={{fontSize:14,fontWeight:600,color:f.strength==="wide"?K.grn:f.strength==="narrow"?K.amb:K.red,fontFamily:fm}}>{f.strength?f.strength.charAt(0).toUpperCase()+f.strength.slice(1)+" Moat":"\u2014"}</div><div style={{fontSize:10,color:K.dim,marginTop:2}}>{f.types.map(function(t){var mt=MOAT_TYPES.find(function(m){return m.id===t});return mt?mt.label:t}).join(", ")}</div></div>
          <div><div style={{fontSize:11,color:K.mid,marginBottom:4}}>Morningstar says</div><div style={{fontSize:14,fontWeight:600,color:f.morningstar==="Wide"?K.grn:f.morningstar==="Narrow"?K.amb:K.red,fontFamily:fm}}>{f.morningstar} Moat</div>{f.mstarSources.length>0&&<div style={{fontSize:10,color:K.dim,marginTop:2}}>{f.mstarSources.join(", ")}</div>}</div>
        </div></div>}
      <div style={{display:"flex",justifyContent:"flex-end",gap:12}}><button style={S.btn} onClick={function(){setModal(null)}}>Cancel</button><button style={S.btnP} onClick={function(){upd(selId,{moat:{types:f.types,strength:f.strength,trend:f.trend,morningstar:f.morningstar,mstarSources:f.mstarSources,notes:f.notes}});setModal(null)}}>Save</button></div></Modal>}

  function DecisionModal(){if(!sel)return null;var did=modal.data;var ex=did?((sel.decisions||[]).find(function(d){return d.id===did})):null;
    var _f=useState({type:ex?ex.type:"buy",date:ex?ex.date:new Date().toISOString().split("T")[0],price:ex?String(ex.price):"",shares:ex?String(ex.shares||""):"",reasoning:ex?ex.reasoning:"",outcome:ex?ex.outcome||"tbd":"tbd",outcomeNote:ex?ex.outcomeNote||"":""}),f=_f[0],setF=_f[1];
    var set=function(k,v){setF(function(p){var n=Object.assign({},p);n[k]=v;return n})};
    var outTag=OUTCOME_TAGS.find(function(o){return o.id===f.outcome});
    function doSave(){if(!f.type||!f.date)return;var entry={type:f.type,date:f.date,price:parseFloat(f.price)||0,shares:parseFloat(f.shares)||0,reasoning:f.reasoning.trim(),outcome:f.outcome,outcomeNote:f.outcomeNote.trim()};
      if(ex){upd(selId,function(c){return Object.assign({},c,{decisions:(c.decisions||[]).map(function(d){return d.id===did?Object.assign({},d,entry):d})})})}
      else{upd(selId,function(c){var decs=(c.decisions||[]).slice();decs.push(Object.assign({id:Date.now()},entry));return Object.assign({},c,{decisions:decs})})}setModal(null)}
    return<Modal title={(ex?"Edit":"Log")+" Decision \u2014 "+sel.ticker} onClose={function(){setModal(null)}} w={520} K={K}>
      <div style={{display:"flex",gap:6,marginBottom:16}}>{DECISION_TYPES.map(function(dt){return<button key={dt.id} onClick={function(){set("type",dt.id)}} style={{flex:1,padding:"8px 0",borderRadius:6,border:"1px solid "+(f.type===dt.id?dt.color+"60":K.bdr),background:f.type===dt.id?dt.color+"15":"transparent",color:f.type===dt.id?dt.color:K.dim,fontSize:11,fontWeight:600,cursor:"pointer",fontFamily:fm}}>{dt.label}</button>})}</div>
      <div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:"0 12px"}}><Inp label="Date" value={f.date} onChange={function(v){set("date",v)}} type="date" K={K}/><Inp label="Price" value={f.price} onChange={function(v){set("price",v)}} type="number" placeholder="0.00" K={K}/><Inp label="Shares" value={f.shares} onChange={function(v){set("shares",v)}} type="number" placeholder="0" K={K}/></div>
      <Inp label="Reasoning" value={f.reasoning} onChange={function(v){set("reasoning",v)}} ta placeholder="Why are you making this decision?" K={K}/>
      <div style={{borderTop:"1px solid "+K.bdr,paddingTop:16,marginTop:4,marginBottom:8}}><div style={{fontSize:11,color:K.dim,letterSpacing:2,textTransform:"uppercase",fontFamily:fm,marginBottom:12}}>Outcome (update later)</div></div>
      <div style={{display:"flex",gap:6,marginBottom:16}}>{OUTCOME_TAGS.map(function(ot){return<button key={ot.id} onClick={function(){set("outcome",ot.id)}} style={{flex:1,padding:"6px 0",borderRadius:6,border:"1px solid "+(f.outcome===ot.id?ot.color+"60":K.bdr),background:f.outcome===ot.id?ot.color+"15":"transparent",color:f.outcome===ot.id?ot.color:K.dim,fontSize:10,fontWeight:600,cursor:"pointer",fontFamily:fm}}>{ot.icon} {ot.label}</button>})}</div>
      {f.outcome!=="tbd"&&<Inp label="Outcome Note" value={f.outcomeNote} onChange={function(v){set("outcomeNote",v)}} placeholder="What happened? What did you learn?" K={K}/>}
      <div style={{display:"flex",justifyContent:"flex-end",gap:12}}>{ex&&<button style={S.btnD} onClick={function(){upd(selId,function(c){return Object.assign({},c,{decisions:(c.decisions||[]).filter(function(d){return d.id!==did})})});setModal(null)}}>Delete</button>}<div style={{flex:1}}/><button style={S.btn} onClick={function(){setModal(null)}}>Cancel</button><button style={S.btnP} onClick={doSave}>Save</button></div></Modal>}
  function ThesisSectionsModal(){if(!sel)return null;var ts=sel.thesisSections||{believe:"",whyNow:"",risks:"",change:""};
    var _f=useState({believe:ts.believe||"",whyNow:ts.whyNow||"",risks:ts.risks||"",change:ts.change||""}),f=_f[0],setF=_f[1];
    var set=function(k,v){setF(function(p){var n=Object.assign({},p);n[k]=v;return n})};
    function doSave(){var combined=[f.believe,f.whyNow,f.risks,f.change].filter(Boolean).join("\n\n");
      // Create version snapshot
      var versions=(sel.thesisVersions||[]).slice();
      if(combined.trim()){versions.push({date:new Date().toISOString().split("T")[0],text:combined.substring(0,500)})}
      upd(selId,{thesisSections:f,thesisNote:combined,thesisVersions:versions.slice(-20)});setModal(null)}
    return<Modal title={"Structured Thesis \u2014 "+sel.ticker} onClose={function(){setModal(null)}} w={600} K={K}>
      <Inp label="What I Believe" value={f.believe} onChange={function(v){set("believe",v)}} ta placeholder="Core thesis: Why is this a good business?" K={K}/>
      <Inp label="Why Now" value={f.whyNow} onChange={function(v){set("whyNow",v)}} ta placeholder="What makes this the right time?" K={K}/>
      <Inp label="Key Risks" value={f.risks} onChange={function(v){set("risks",v)}} ta placeholder="What could go wrong?" K={K}/>
      <Inp label="What Would Change My Mind" value={f.change} onChange={function(v){set("change",v)}} ta placeholder="Specific triggers that would make you sell" K={K}/>
      {sel.thesisVersions&&sel.thesisVersions.length>0&&<div style={{borderTop:"1px solid "+K.bdr,paddingTop:12,marginTop:4}}>
        ...
        <div style={{maxHeight:120,overflowY:"auto"}}>{sel.thesisVersions.slice().reverse().map(function(v,i){return<div key={i} style={{fontSize:11,color:K.mid,marginBottom:6,paddingLeft:8,borderLeft:"2px solid "+K.bdr}}>
          <span style={{fontFamily:fm,color:K.dim}}>{v.date}</span> \u2014 {v.text.substring(0,80)}{v.text.length>80?"...":""}</div>})}</div></div>}
      <div style={{display:"flex",justifyContent:"flex-end",gap:12,marginTop:12}}><button style={S.btn} onClick={function(){setModal(null)}}>Cancel</button><button style={S.btnP} onClick={doSave}>Save & Snapshot</button></div></Modal>}
  function BiasChecklistModal(){if(!sel)return null;var bc=sel.biasChecklist||{completedAt:null,flags:[],notes:""};
    var BIASES=[{id:"confirmation_bias",label:"Confirmation Bias",q:"Am I only seeking info that supports my thesis?"},{id:"anchoring",label:"Anchoring",q:"Am I fixated on a specific price or narrative?"},{id:"fomo",label:"FOMO",q:"Am I buying because others are, or because of my own research?"},{id:"sunk_cost",label:"Sunk Cost",q:"Am I holding because I should, or because I already invested?"},{id:"recency",label:"Recency Bias",q:"Am I overweighting the most recent quarter?"},{id:"lollapalooza",label:"Lollapalooza Effect",q:"Are multiple biases compounding at once?"}];
    var _flags=useState((bc.flags||[]).slice()),flags=_flags[0],setFlags=_flags[1];
    var _notes=useState(bc.notes||""),notes=_notes[0],setNotes=_notes[1];
    function toggle(id){setFlags(function(p){var i=p.indexOf(id);if(i>=0){var n=p.slice();n.splice(i,1);return n}return p.concat([id])})}
    return<Modal title={"Pre-Flight Checklist \u2014 "+sel.ticker} onClose={function(){setModal(null)}} w={540} K={K}>
      <div style={{fontSize:12,color:K.mid,marginBottom:16,lineHeight:1.6}}>Review each cognitive bias before deploying capital. Flag any that apply to your current thinking.</div>
      {BIASES.map(function(b){var flagged=flags.indexOf(b.id)>=0;return<div key={b.id} style={{display:"flex",alignItems:"flex-start",gap:10,padding:"10px 14px",marginBottom:6,background:flagged?K.amb+"08":K.bg,border:"1px solid "+(flagged?K.amb+"30":K.bdr),borderRadius:8,cursor:"pointer"}} onClick={function(){toggle(b.id)}}>
        <div style={{width:18,height:18,borderRadius:4,border:"2px solid "+(flagged?K.amb:K.bdr),background:flagged?K.amb:"transparent",display:"flex",alignItems:"center",justifyContent:"center",flexShrink:0,marginTop:1}}>{flagged&&<span style={{color:"#fff",fontSize:11,fontWeight:700}}>\u2713</span>}</div>
        <div><div style={{fontSize:12,fontWeight:500,color:flagged?K.amb:K.txt}}>{b.label}</div><div style={{fontSize:11,color:K.dim,marginTop:2}}>{b.q}</div></div></div>})}
      <Inp label="Notes" value={notes} onChange={setNotes} ta placeholder="Additional pre-flight observations..." K={K}/>
      <div style={{display:"flex",justifyContent:"flex-end",gap:12,marginTop:8}}><button style={S.btn} onClick={function(){setModal(null)}}>Cancel</button><button style={S.btnP} onClick={function(){upd(selId,{biasChecklist:{completedAt:new Date().toISOString(),flags:flags,notes:notes.trim()}});setModal(null)}}>Complete Checklist</button></div></Modal>}
function ManualEarningsModal(){if(!sel)return null;
  ...
  function renderModal(){if(!modal)return null;var map={add:AddModal,edit:EditModal,thesis:ThesisModal,kpi:KpiModal,result:ResultModal,del:DelModal,doc:DocModal,position:PositionModal,conviction:ConvictionModal,moat:MoatModal,decision:DecisionModal,thesisSections:ThesisSectionsModal,biasChecklist:BiasChecklistModal,manualEarnings:ManualEarningsModal};var C=map[modal.type];return C?<C/>:null}
  ...
  function DetailView(){if(!sel)return null;var c=sel;var h=gH(c.kpis);var cs=checkSt[c.id];var pos=c.position||{};var conv=c.conviction||0;
    return<div style={{padding:"0 32px 60px",maxWidth:900}}>
      ...
      {c.thesisNote&&!(c.thesisSections&&(c.thesisSections.believe||c.thesisSections.whyNow))&&<div style={{background:K.card,border:"1px solid "+K.bdr,borderRadius:10,padding:"14px 20px",marginBottom:20}}><div style={{display:"flex",justifyContent:"space-between",alignItems:"center"}}><div style={S.sec}>Investment Thesis</div><button style={Object.assign({},S.btn,{padding:"4px 10px",fontSize:10})} onClick={function(){setModal({type:"thesisSections"})}}>Upgrade to Structured</button></div><div style={{fontSize:13,color:K.mid,lineHeight:1.6,cursor:"pointer"}} onClick={function(){setModal({type:"thesis"})}}>{c.thesisNote}</div></div>}
      ...
      {c.earningsDate&&c.earningsDate!=="TBD"&&dU(c.earningsDate)>=0&&dU(c.earningsDate)<=14&&<div style={{background:K.amb+"08",border:"1px solid "+K.amb+"25",borderRadius:10,padding:"14px 20px",marginBottom:16}}>
        <div style={{display:"flex",alignItems:"center",gap:8,marginBottom:10}}><span style={{fontSize:14}}>{"\uD83D\uDCCB"}</span><span style={{fontSize:12,fontWeight:600,color:K.amb,fontFamily:fm}}>Pre-Earnings Briefing</span><span style={{fontSize:10,color:K.amb,fontFamily:fm,marginLeft:"auto"}}>{dU(c.earningsDate)}d until {fD(c.earningsDate)} {c.earningsTime}</span></div>
        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10,marginBottom:10}}>
          <div style={{background:K.card,borderRadius:8,padding:"10px 14px"}}><div style={{fontSize:10,color:K.dim,fontFamily:fm}}>CONVICTION</div><div style={{fontSize:16,fontWeight:600,color:conv>=8?K.grn:conv>=5?K.amb:K.red,fontFamily:fm}}>{conv||"\u2014"}/10</div></div>
          <div style={{background:K.card,borderRadius:8,padding:"10px 14px"}}><div style={{fontSize:10,color:K.dim,fontFamily:fm}}>KPIs TO WATCH</div><div style={{fontSize:16,fontWeight:600,color:K.txt,fontFamily:fm}}>{c.kpis.length}</div></div></div>
        {c.kpis.length>0&&<div style={{marginBottom:10}}>{c.kpis.map(function(k){return<div key={k.id} style={{display:"flex",alignItems:"center",gap:8,padding:"6px 0",borderBottom:"1px solid "+K.bdr+"40"}}>
        ...
        {c.thesisSections&&c.thesisSections.believe&&<div style={{fontSize:12,color:K.mid,lineHeight:1.5,padding:"8px 12px",background:K.card,borderRadius:6}}><strong style={{color:K.txt}}>Thesis:</strong> {c.thesisSections.believe.substring(0,200)}</div>}
        {c.moat&&c.moat.trend==="Eroding"&&<div style={{marginTop:8,fontSize:11,color:K.red}}>{"\u26A0\uFE0F"} Moat trend is Eroding \u2014 watch for further deterioration.</div>}
      </div>}
      ...
      {/* ── Moat Assessment ── */}
      <div style={{marginBottom:20}}>
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:12}}><div style={S.sec}>Moat Assessment</div><button style={Object.assign({},S.btn,{padding:"5px 12px",fontSize:11})} onClick={function(){setModal({type:"moat"})}}>Edit Moat</button></div>
        {c.moat&&c.moat.types&&c.moat.types.length>0?<div style={{background:K.card,border:"1px solid "+K.bdr,borderRadius:10,padding:"14px 20px"}}>
          <div style={{display:"flex",gap:8,marginBottom:12,flexWrap:"wrap"}}>{c.moat.types.map(function(t){var mt=MOAT_TYPES.find(function(m){return m.id===t});return mt?<span key={t} style={{display:"inline-flex",alignItems:"center",gap:4,padding:"4px 10px",background:K.grn+"10",border:"1px solid "+K.grn+"30",borderRadius:6,fontSize:11,color:K.grn,fontFamily:fm}}>{mt.icon} {mt.label}</span>:null})}</div>
          <div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:12,marginBottom:12}}>
            <div><div style={{fontSize:10,color:K.dim,fontFamily:fm}}>YOUR RATING</div><div style={{fontSize:14,fontWeight:600,color:c.moat.strength==="wide"?K.grn:c.moat.strength==="narrow"?K.amb:K.red,fontFamily:fm}}>{c.moat.strength?c.moat.strength.charAt(0).toUpperCase()+c.moat.strength.slice(1)+" Moat":"\u2014"}</div></div>
            <div><div style={{fontSize:10,color:K.dim,fontFamily:fm}}>MORNINGSTAR</div><div style={{fontSize:14,fontWeight:600,color:c.moat.morningstar==="Wide"?K.grn:c.moat.morningstar==="Narrow"?K.amb:K.mid,fontFamily:fm}}>{c.moat.morningstar||"\u2014"}</div></div>
            <div><div style={{fontSize:10,color:K.dim,fontFamily:fm}}>TREND</div><div style={{fontSize:14,fontWeight:600,color:c.moat.trend==="Strengthening"?K.grn:c.moat.trend==="Eroding"?K.red:K.mid,fontFamily:fm}}>{c.moat.trend==="Strengthening"?"\u25B2":c.moat.trend==="Eroding"?"\u25BC":"\u2500"} {c.moat.trend||"\u2014"}</div></div></div>
          {c.moat.notes&&<div style={{fontSize:12,color:K.dim,lineHeight:1.5,fontStyle:"italic",borderTop:"1px solid "+K.bdr,paddingTop:10}}>{c.moat.notes}</div>}
        </div>:<div style={{background:K.card,border:"1px dashed "+K.bdr,borderRadius:10,padding:24,textAlign:"center",fontSize:12,color:K.dim,cursor:"pointer"}} onClick={function(){setModal({type:"moat"})}}>No moat assessment yet. Click to add one.</div>}</div>
      {/* ── Structured Thesis (4 sections) ── */}
      {c.thesisSections&&(c.thesisSections.believe||c.thesisSections.whyNow||c.thesisSections.risks||c.thesisSections.change)?<div style={{background:K.card,border:"1px solid "+K.bdr,borderRadius:10,padding:"14px 20px",marginBottom:20,cursor:"pointer"}} onClick={function(){setModal({type:"thesisSections"})}}>
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:12}}><div style={S.sec}>Structured Thesis</div>{c.thesisVersions&&c.thesisVersions.length>0&&<span style={{fontSize:10,color:K.dim,fontFamily:fm}}>v{c.thesisVersions.length}</span>}</div>
        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:12}}>
          {c.thesisSections.believe&&<div style={{background:K.bg,borderRadius:8,padding:"10px 14px"}}><div style={{fontSize:10,color:K.grn,fontWeight:600,letterSpacing:1,textTransform:"uppercase",fontFamily:fm,marginBottom:4}}>What I Believe</div><div style={{fontSize:12,color:K.mid,lineHeight:1.5}}>{c.thesisSections.believe}</div></div>}
          {c.thesisSections.whyNow&&<div style={{background:K.bg,borderRadius:8,padding:"10px 14px"}}><div style={{fontSize:10,color:K.blue,fontWeight:600,letterSpacing:1,textTransform:"uppercase",fontFamily:fm,marginBottom:4}}>Why Now</div><div style={{fontSize:12,color:K.mid,lineHeight:1.5}}>{c.thesisSections.whyNow}</div></div>}
          {c.thesisSections.risks&&<div style={{background:K.bg,borderRadius:8,padding:"10px 14px"}}><div style={{fontSize:10,color:K.red,fontWeight:600,letterSpacing:1,textTransform:"uppercase",fontFamily:fm,marginBottom:4}}>Key Risks</div><div style={{fontSize:12,color:K.mid,lineHeight:1.5}}>{c.thesisSections.risks}</div></div>}
          {c.thesisSections.change&&<div style={{background:K.bg,borderRadius:8,padding:"10px 14px"}}><div style={{fontSize:10,color:K.amb,fontWeight:600,letterSpacing:1,textTransform:"uppercase",fontFamily:fm,marginBottom:4}}>Would Change My Mind</div><div style={{fontSize:12,color:K.mid,lineHeight:1.5}}>{c.thesisSections.change}</div></div>}
        </div>
      </div>:<div style={{background:K.card,border:"1px dashed "+K.bdr,borderRadius:10,padding:24,textAlign:"center",marginBottom:20}}><div style={{fontSize:12,color:K.dim,marginBottom:8}}>No structured thesis yet</div><button style={Object.assign({},S.btn,{padding:"6px 14px",fontSize:11})} onClick={function(){setModal({type:"thesisSections"})}}>Write Structured Thesis</button></div>}
      {/* ── Decision Journal ── */}
      <div style={{marginBottom:20}}>
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:12}}><div style={S.sec}>Decision Journal</div>
          <div style={{display:"flex",gap:6}}>{c.biasChecklist&&c.biasChecklist.completedAt&&<span style={{fontSize:10,color:K.grn,fontFamily:fm,padding:"4px 8px",background:K.grn+"10",borderRadius:4}}>\u2713 Bias check done</span>}
          <button style={Object.assign({},S.btn,{padding:"5px 12px",fontSize:11})} onClick={function(){setModal({type:"biasChecklist"})}}>Bias Check</button>
          <button style={Object.assign({},S.btnP,{padding:"5px 12px",fontSize:11})} onClick={function(){setModal({type:"decision"})}}>+ Log Decision</button></div></div>
        {(function(){var decs=c.decisions||[];var goodCalls=decs.filter(function(d){return d.outcome==="good"}).length;var totalRated=decs.filter(function(d){return d.outcome&&d.outcome!=="tbd"}).length;var qualityPct=totalRated>0?Math.round(goodCalls/totalRated*100):null;
          return decs.length===0?<div style={{background:K.card,border:"1px dashed "+K.bdr,borderRadius:10,padding:24,textAlign:"center",fontSize:12,color:K.dim,cursor:"pointer"}} onClick={function(){setModal({type:"decision"})}}>No decisions logged yet. Start your investment journal.</div>:
          <div style={{background:K.card,border:"1px solid "+K.bdr,borderRadius:10,overflow:"hidden"}}>
            {qualityPct!==null&&<div style={{padding:"10px 20px",borderBottom:"1px solid "+K.bdr,display:"flex",justifyContent:"space-between",alignItems:"center"}}>
              <span style={{fontSize:11,color:K.mid}}>Decision Quality</span>
              <span style={{fontSize:13,fontWeight:600,color:qualityPct>=70?K.grn:qualityPct>=50?K.amb:K.red,fontFamily:fm}}>{qualityPct}% accuracy ({goodCalls}/{totalRated} good calls)</span></div>}
            {decs.slice().reverse().map(function(d){var dt=DECISION_TYPES.find(function(t){return t.id===d.type});var ot=OUTCOME_TAGS.find(function(o){return o.id===d.outcome});
              return<div key={d.id} style={{padding:"12px 20px",borderBottom:"1px solid "+K.bdr,cursor:"pointer"}} onClick={function(){setModal({type:"decision",data:d.id})}}>
                <div style={{display:"flex",alignItems:"center",gap:8,marginBottom:6}}>
                  <span style={{fontSize:10,fontWeight:700,color:dt?dt.color:K.mid,fontFamily:fm,padding:"2px 8px",background:(dt?dt.color:K.mid)+"15",borderRadius:4}}>{d.type.toUpperCase()}</span>
                  <span style={{fontSize:11,color:K.dim,fontFamily:fm}}>{d.date}</span>
                  {d.price>0&&<span style={{fontSize:11,color:K.mid,fontFamily:fm}}>@ ${d.price}</span>}
                  {d.shares>0&&<span style={{fontSize:11,color:K.dim,fontFamily:fm}}>{d.shares} shares</span>}
                  <span style={{flex:1}}/>
                  {ot&&ot.id!=="tbd"&&<span style={{fontSize:10,fontWeight:600,color:ot.color,fontFamily:fm,padding:"2px 8px",background:ot.color+"15",borderRadius:4}}>{ot.icon} {ot.label}</span>}
                </div>
                {d.reasoning&&<div style={{fontSize:12,color:K.mid,lineHeight:1.5}}>{d.reasoning}</div>}
                {d.outcomeNote&&<div style={{fontSize:11,color:K.dim,marginTop:4,fontStyle:"italic"}}>{d.outcomeNote}</div>}</div>})}
          </div>})()}</div>
      <NarrativeDrift company={c}/><QAEvasion company={c}/>
      {c.thesisVersions&&c.thesisVersions.length>1&&<div style={{marginBottom:20}}>
        <div style={S.sec}>Thesis Timeline</div>
        <div style={{background:K.card,border:"1px solid "+K.bdr,borderRadius:10,padding:"14px 20px"}}>
          {c.thesisVersions.slice().reverse().map(function(v,i){return<div key={i} style={{display:"flex",gap:12,paddingBottom:12,marginBottom:12,borderBottom:i<c.thesisVersions.length-1?"1px solid "+K.bdr:"none"}}>
            <div style={{width:2,background:K.acc,borderRadius:1,flexShrink:0}}/>
            <div><div style={{fontSize:11,fontWeight:600,color:K.acc,fontFamily:fm}}>v{c.thesisVersions.length-i} \u2014 {v.date}</div>
              <div style={{fontSize:12,color:K.mid,lineHeight:1.5,marginTop:4}}>{v.text}</div></div></div>})}</div></div>}
  ...
  function OwnersHub(){
    ...
    return<div style={{padding:"0 32px 60px",maxWidth:1000}}>
      ...
      {(function(){var os=calcOwnerScore(cos);var allDecisions=[];cos.forEach(function(c){(c.decisions||[]).forEach(function(d){allDecisions.push(Object.assign({},d,{ticker:c.ticker}))})});
        var goodCalls=allDecisions.filter(function(d){return d.outcome==="good"}).length;var totalRated=allDecisions.filter(function(d){return d.outcome&&d.outcome!=="tbd"}).length;
        return<div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:12,marginBottom:24}}>
          <div style={{background:K.card,border:"1px solid "+K.bdr,borderRadius:10,padding:"14px 20px",textAlign:"center"}}>
            <div style={{fontSize:10,color:K.dim,letterSpacing:2,textTransform:"uppercase",fontFamily:fm,marginBottom:6}}>Owner's Score</div>
            <div style={{fontSize:32,fontWeight:700,color:os.total>=75?K.grn:os.total>=50?K.amb:K.red,fontFamily:fm}}>{os.total}</div>
            <div style={{fontSize:11,color:K.dim}}>out of 100</div></div>
          <div style={{background:K.card,border:"1px solid "+K.bdr,borderRadius:10,padding:"14px 20px",textAlign:"center"}}>
            <div style={{fontSize:10,color:K.dim,letterSpacing:2,textTransform:"uppercase",fontFamily:fm,marginBottom:6}}>Decision Quality</div>
            <div style={{fontSize:32,fontWeight:700,color:totalRated>0?(goodCalls/totalRated>=.7?K.grn:goodCalls/totalRated>=.5?K.amb:K.red):K.dim,fontFamily:fm}}>{totalRated>0?Math.round(goodCalls/totalRated*100)+"%":"\u2014"}</div>
            <div style={{fontSize:11,color:K.dim}}>{totalRated>0?goodCalls+"/"+totalRated+" good calls":"No decisions rated yet"}</div></div>
          <div style={{background:K.card,border:"1px solid "+K.bdr,borderRadius:10,padding:"14px 20px",textAlign:"center"}}>
            <div style={{fontSize:10,color:K.dim,letterSpacing:2,textTransform:"uppercase",fontFamily:fm,marginBottom:6}}>Total Decisions</div>
            <div style={{fontSize:32,fontWeight:700,color:K.txt,fontFamily:fm}}>{allDecisions.length}</div>
            <div style={{fontSize:11,color:K.dim}}>across {cos.filter(function(c){return(c.decisions||[]).length>0}).length} companies</div></div>
        </div>})()}
      {/* Filters */}
      ...
      return<div style={{background:K.card,border:"1px solid "+K.bdr,borderRadius:12,padding:"20px 24px",marginBottom:20}}>
        <div style={{display:"flex",alignItems:"center",gap:16,marginBottom:16}}>
          <div style={{position:"relative",width:72,height:72}}>
            <svg width="72" height="72" viewBox="0 0 72 72"><circle cx="36" cy="36" r="30" fill="none" stroke={K.bdr} strokeWidth="6"/><circle cx="36" cy="36" r="30" fill="none" stroke={os.total>=75?K.grn:os.total>=50?K.amb:os.total>=25?K.blue:K.red} strokeWidth="6" strokeDasharray={Math.round(os.total/100*188.5)+" 188.5"} strokeLinecap="round" transform="rotate(-90 36 36)"/></svg>
            <div style={{position:"absolute",inset:0,display:"flex",alignItems:"center",justifyContent:"center",flexDirection:"column"}}><div style={{fontSize:22,fontWeight:700,color:os.total>=75?K.grn:os.total>=50?K.amb:os.total>=25?K.blue:K.red,fontFamily:fm,lineHeight:1}}>{os.total}</div></div></div>
          <div style={{flex:1}}><div style={{fontSize:10,letterSpacing:3,textTransform:"uppercase",color:K.dim,marginBottom:4,fontFamily:fm}}>Owner's Score</div>
            <div style={{fontSize:14,color:K.txt,fontWeight:500}}>{os.total>=80?"Exceptional discipline":os.total>=60?"Strong foundation":os.total>=40?"Building momentum":"Getting started"}</div>
            <div style={{fontSize:11,color:K.dim,marginTop:2}}>Track your investment process quality</div></div></div>
        <div style={{display:"grid",gridTemplateColumns:"repeat(6,1fr)",gap:8}}>{cats.map(function(cat){var pct=Math.round(cat.pts/cat.max*100);return<div key={cat.label} style={{textAlign:"center"}}>
          <div style={{height:4,borderRadius:2,background:K.bdr,marginBottom:6,overflow:"hidden"}}><div style={{height:"100%",width:pct+"%",borderRadius:2,background:cat.color}}/></div>
          <div style={{fontSize:9,color:K.dim,fontFamily:fm}}>{cat.label}</div>
          <div style={{fontSize:11,fontWeight:600,color:cat.color,fontFamily:fm}}>{cat.pts}/{cat.max}</div></div>})}</div></div>})()}
